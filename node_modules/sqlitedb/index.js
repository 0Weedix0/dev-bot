...
/**
 * run函数接受一个字符串参数，其中?表示占位符，占位符的值必须通过一个数组传递进来调用者提供了一个回调函数，然后通过这个回调函数来声明错误
...
exports.edit = function(ts, author, note, callback) {
    db.run("UPDATE notes SET ts = ?, author = ?, note = ? WHERE ts = ?", [ts, author, note, ts],
        function(err) {
            if (err) {
...
exports.findNoteById = function(ts, callback) {
    var didOne = false;
    db.each("SELECT * FROM notes WHERE ts = ?", [ts],
        function(err, row) {
            if (err) {
                util.log('FAIL to retrieve row ' + err);
                callback(err, null);
